...
class ListBlobs {
  ...
  process(request, res) {
    ...
    storageManager.listBlobs(request, query).then((response) => {
      ...
      const blobPrefixes = [];
      const transformedModel = this._transformBlobList(
        response.payload,
        query,
        blobPrefixes,
        response.nextMarker
      ...
      xmlDoc = xmlDoc.replace(
        `<EnumerationResults>`,
        `<EnumerationResults ServiceEndpoint="http://0.0.0.0:10000/devstoreaccount1" ContainerName="${
  ...
  _transformBlobList(blobList, query, blobPrefixes, nextMarker) {
    ...
    query.maxresults === undefined
      ? delete xmlBlobListModel.MaxResults
      : (xmlBlobListModel.MaxResults = query.maxresults);
    query.delimiter === undefined
      ? delete xmlBlobListModel.Delimiter
      : (xmlBlobListModel.Delimiter = query.delimiter);
    if (nextMarker > 0) {
    ...
    if (query.delimiter !== undefined) {
      blobList = blobList.filter((blob) => {
        ...
        const restOfName = blobName.substr(
          query.prefix.length,
          blobName.length
        );
        ...
        if (!keep) {
          if (restOfName.indexOf(query.delimiter) === -1) {
            ...
            const blobPrefix = `${query.prefix}${
              restOfName.split(query.delimiter)[0]
              }${query.delimiter}`;
    ...
    if (query.marker) {
      blobList.length === 0
        ? delete xmlBlobListModel.Marker
        : (xmlBlobListModel.Marker = blobList[0].original.name);
    }
    ...
    for (const blob of blobList) {
      ...
      if (blob.original.copyId) {
        ...
        modelBlob.Properties.CopyCompletionTime =
          blob.original.copyCompletionTime;
        blob.original.copyStatusDescription
          ? (modelBlob.Properties.CopyStatusDescription =
            blob.original.copyStatusDescription)
      ...
      modelBlob.Properties["Content-Type"] = blob.original.contentType
        ...
        : {};
      modelBlob.Properties["Content-Encoding"] = blob.original.contentEncoding
        ? blob.original.contentEncoding
        : {};
      modelBlob.Properties["Content-MD5"] = blob.original.md5
        ? blob.original.md5
        : {};
      modelBlob.Properties["Content-Length"] = blob.original.size;
      modelBlob.Properties["Cache-Control"] = blob.original.cacheControl
        ? blob.original.cacheControl
        : {};
      modelBlob.Properties["Content-Language"] = blob.original.contentLanguage
        ? blob.original.contentLanguage
        : {};
      modelBlob.Properties["Content-Disposition"] = blob.original
        .contentDisposition
        ? blob.original.contentDisposition
        : {};
      modelBlob.Properties.BlobType = blob.original.entityType;
      modelBlob.Properties["x-ms-blob-sequence-number"] = blob.original
        .sequenceNumber
        ? blob.original.sequenceNumber
        : {};
      modelBlob.Properties.LeaseStatus = [
        LeaseStatus.AVAILABLE,
        LeaseStatus.BROKEN,
        LeaseStatus.EXPIRED,
      ].includes(blob.original.leaseState)
        ? "unlocked"
        : "locked";
      modelBlob.Properties.LeaseState = blob.original.leaseState;
      if (blob.original.leaseState === LeaseStatus.LEASED) {
        modelBlob.Properties.LeaseDuration =
          blob.original.leaseDuration === -1 ? "infinite" : "fixed";
      } else {
